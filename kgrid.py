...
def calc_grid(cutoff_length, filename='geometry.in',
              filetype=False, pretty_print=False):
...
def calc_kpt_tuple(atoms, cutoff_length=10):
    """Calculate Moreno-Soler k-point grid for ASE atoms object"""
...
if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option("-c", "--cutoff-length", action="store", type="float",
                      dest="cutoff_length", default=10.0,
                      help="Set length cutoff in Angstroms [default: 10]")
    parser.add_option("-f", "--file", action="store", type="string",
                      dest="file", default="geometry.in",
    ...
    calc_grid(options.cutoff_length, filename=options.file,
              filetype=options.type, pretty_print=True)
